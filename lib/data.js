...
/**
 * Packs basic information about a player profile.
 */
export function packPlayerData(player) {
  return {
 ...
 */
export async function getProfileWithRank(rankings, profile, gateways, {getMatches = false} = {}) {
  ...
  const latestMatchesSorted = getMatches ? await getProfileLadderGames(profile) : []
  ...
  return {
    ...
    lastActivity: latestMatchesSorted.length ? latestMatchesSorted[0].matchTimestamp : null,
